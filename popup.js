  ...
  function renderItems(items) {
    try {
      ...
      list.innerHTML = items.map(function(i) {
        ...
        return '<div class="item" data-url="' + escHtml(i.url) + '" data-id="' + (i.id || '') + '">' +
          '<div class="item-top"><div class="item-title">' + escHtml(title) + '</div><button class="item-delete" title="delete">&times;</button></div>' +
          note +
      ...
      list.querySelectorAll(".item").forEach(function(el) {
        ...
        var delBtn = el.querySelector(".item-delete");
        if (delBtn) {
          delBtn.addEventListener("click", function(e) {
            ...
            if (!id) return;
            try {
              chrome.runtime.sendMessage({ action: "deleteEntry", id: Number(id) }, function(resp) {
                if (chrome.runtime.lastError || !resp || !resp.ok) return;
                el.style.opacity = "0";
                el.style.transition = "opacity 0.15s";
                setTimeout(function() { el.remove(); }, 150);
                var t = parseInt(countEl.textContent) || 0;
                if (t > 0) countEl.textContent = (t - 1) + " saved";
              });
            } catch(e) {}
          });
        }
